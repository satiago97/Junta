    ...
    @include('backoffice/sidebar')
        ...
        <div class="main-content">
            ...
            <div class="container" style="margin-top: 20px;">
                <div class="row">
                    <div class="mainText">
                    <form action="{{ route('trilho.store') }}" method="post" enctype="multipart/form-data" id="trilhosForm">
        <!-- Add CSRF Token -->
        @csrf
    <div class="form-group">
        <h1>Adicionar Trilho</h1>
    </div>
    <div class="form-group">
        <div class="row">
        <div class="col-5">
...
<div class="container" style="margin-top: 20px;">
    <div class="row">
        <div class="mainText">
    <h1 class="TituloLista">Lista de Trilhos</h1>       
    <table class="table TablePos">
        <thead>
            <tr>
        ...
        <tbody id="tests-table">
            @foreach ($trilho as $item)
            <tr scope="row">
                <td>{{$item->nome}}</td>
                <td>{{$item->descricao}}</td>
                <td>
                  <a href="{{ route('edit-trilho',[$item->id]) }}" class="btn btn-sm btn-primary hidden">Edit</a>
                  <form action="{{ route('delete-trilho',[$item->id]) }}" method="post" id="form1">
                    @method('DELETE')
                    @csrf
