...
<div id="preloader">
    ...
    <div class="main-content">
            ...
            <a id="containerButton" href="{{route('insereRedeEscolar')}}">Adicionar</a>
            </div>
    ...
    <table class="fl-table">
      ...
      @foreach($redeEscolar as $item)
        ...
        <tr>
            <td>{{$item->nome}}</td>
            <td>{{$item->morada}}</td>
            <td><a href="">{{$item->foto}}</a></td>
            <td>
            <a href="{{ url('open-inserirRedeEscolar/'.$item->id)}}" class="btn btn-primary a-btn-slide-text">
            <span class="glyphicon glyphicon-edit" aria-hidden="true"></span>
            <span><strong>Editar</strong></span>     
            </a>
            <form action="{{route('delete-inserirRedeEscolar', [$item->id])}}" method="post" id="form1">
                @method('DELETE')
                @csrf
