...
<div id="preloader">
    ...
    <div class="main-content">
            ...
            <a id="containerButton" href="{{route('insereJornal')}}">Adicionar</a>
            </div>
...
<div class="table-wrapper">
    <table class="fl-table">
        ...
        <tbody>
        <tr>@foreach($jornal as $item)
            <td>{{$item->titulo}}</td>
            <td>{{$item->data}}</td>
            <td><a href="/jornal/{{$item->foto}}">{{$item->foto}}</td>
         
            <td><a href="/jornal/{{$item->pdf}}">{{$item->pdf}}</a></td>
            <td>
            <a href="{{ url('open-inserirJornal/'.$item->id)}}" class="btn btn-primary a-btn-slide-text">
            <span class="glyphicon glyphicon-edit" aria-hidden="true"></span>
            <span><strong>Editar</strong></span>     
            </a>
            <form action="{{route('delete-inserirJornal', [$item->id])}}" method="post" id="form1">
                @method('DELETE')
                @csrf
